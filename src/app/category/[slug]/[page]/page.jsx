...
const getProducts = async (id, pg) => {
  ...
  try{
        const res = await fetch(`http://localhost:8080/api/category/searchpagination/${id}/${pg}`, {cache:"no-store"})
        return res.json()
  }catch(err){
    ...
    <div>
      ...
      <div className="pagenation">
        <div className="prevpage">
          ...
          {(prevPage > 0) && <Link href={`/category/${slug}/${prevPage}`}><div className="wordleft">Prev</div></Link> }
        </div>
        <div className="nextpage">
        <Link href={`/category/${slug}/${nextNumber}`} prefetch={false}><div className="wordleft">Next</div></Link>

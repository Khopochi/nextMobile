      ...
      const handleRegister = async () => {
        ...
        if (credentials.confirm?.trim() && credentials.password?.trim() && value?.trim() && (value?.trim().length >= 9) &&  credentials.firstname?.trim()) {
          ...
          try{
            ...
            const res = await axios.post("https://api.jiabaili.shop/api/user/temp", tempuser)
            setLoader(false)
            if(res.data.phonenumber){
              localStorage.setItem('jiabailiuser', JSON.stringify(data));
              router.push("/otp")
        ...
        <div className="title">
            ...
            <div className="fname">
                ...
                <div className='passadvice'>
                    <span> <img src="/image/alert-circle.svg" alt="" /> </span>
            ...
            <div className="fname">
                ...
                By creating an account, you agree to <span  onClick={()=>router.push("/about")} className="link">JiaBaiLi&apos; Supermarket Conditions of Use</span> and <span onClick={()=>router.push("/about")} className="link">Privacy Notice</span>.
                </div>
            </div>
            ...
            <div className="fname">
                <div className="termsandconditions1">
                    Already have an Account?  <Link href="/login/"><span className="link">Sign In</span> </Link>
                </div>
            </div>
        </div>
